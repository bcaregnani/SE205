...
thread_pool_t *thread_pool_init(int core_pool_size, int max_pool_size) {
  ...
  // Add mutex and condition variable to protect thread pool against
  // concurrent accesses
  pthread_mutex_init(&thread_pool->pool_mutex, NULL);
  pthread_cond_init(&thread_pool->pool_cv, NULL);
  return thread_pool;
}
...
void pool_thread_terminate(thread_pool_t *thread_pool) {
  ...
  */
  ...
  // Signal that the thread pool is empty
  if (thread_pool->size == 0)
  {
    pthread_cond_signal(&thread_pool->pool_cv);
  };
...
void wait_thread_pool_empty(thread_pool_t *thread_pool) {
  ...
  */
  ...
  {
    pthread_cond_wait(&thread_pool->pool_cv, &thread_pool->pool_mutex);
  };
  pthread_mutex_unlock(&thread_pool->pool_mutex);

  // Now this looses sense to have it here
  if (thread_pool->size != 0)

...
future_t *submit_callable(executor_t *executor, callable_t *callable) {
  ...
  */
  pthread_mutex_init(&future->future_mutex, NULL);
  pthread_cond_init(&future->future_cv, NULL);
  ...
  */
  if (pool_thread_create(executor->thread_pool, pool_thread_main, future, 1))
    return future;
...
void *get_callable_result(future_t *future) {
  ...
  */
  ...
  {
    pthread_cond_wait( &future->future_cv,&future->future_mutex );
  };
  ...
  while (!terminate) {
    ...
    */
    if (callable->period == 0) {
      ...
      */
      future->completed = 1;
      pthread_cond_signal(&future->future_cv);
      ...
      */
        ...
        */
        ...
        // Adding new time value and specifications
        struct timespec future_ts;
        struct timeval future_tv;

        // Getting the appropiate time spec in absolute delay
        gettimeofday(&future_tv, NULL);
        TIMEVAL_TO_TIMESPEC(&future_tv, &future_ts);
        add_millis_to_timespec(&future_ts, executor->keep_alive_time);
        
        future = (future_t *) protected_buffer_poll(executor->futures, &future_ts);

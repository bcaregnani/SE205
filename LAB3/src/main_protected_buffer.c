...
int main(int argc, char *argv[]) {
  ...
  pthread_t *tasksArray[n_consumers+n_producers];
  ...
  for (i = 0; i < n_consumers; i++) {
    // Begin my code
    tasks = malloc(sizeof(pthread_t));
    tasksArray[i] = tasks;
    data = malloc(sizeof(int));
    *data = i;
    pthread_create(tasks, NULL, main_consumer, data);
    // End my code
  ...
  for (i = n_consumers; i < n_producers + n_consumers; i++) {
    ...
    data = malloc(sizeof(int));
    *data = i;
    pthread_create(tasks, NULL, main_producer, data);
    // End my code
  ...
  for (i = 0; i < n_consumers + n_producers; i++) {
    pthread_join(*tasksArray[i], NULL);
  }
}

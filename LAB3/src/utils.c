...
// Mutex for managing delays
pthread_mutex_t delay_mutex;
pthread_cond_t delay_condvar;
...
/**
 * Q: Explain how this procedure can make the task wait well beyond the deadline 
 *    if the execution of gettimeofday is not immediately followed by the 
 *    execution of nanosleep. 
 * A: To set an example lets think we have two consumers threads, lets call them 
 *    thread1 and thread2. Suppose the case when thread1 is executing 
 *    "delay_until()" after getting the time of the day but before evaluating 
 *    nanosleep, at this moment thread2 executes "delay_until()" in the 
 *    instruction nanosleep(), what happens here is that the total delay is 
 *    going to be the delay of thread2 plus the relative time of thread1 
 *    (that remains unchanged). This time is more than the desired delay time.
 * 
 * 
 * 
 * 
